/* File:      lab3part3.c
 * Purpose:   Build a histogram from random data using OpenMP
 * 
 * Compile:   gcc ... 
 *                  
 * Run:       ./lab3part3 <bin_count> <min_meas> <max_meas> <data_count> <thread_count>
 *                  
 *
 * Input:     None
 * Output:    A histogram with X's showing the number of measurements in each bin
 *            
 *
 * Notes:
...
int main(int argc, char* argv[]) {
    ...
    GET_TIME(start);
    #pragma omp parallel num_threads(thread_count) private(bin)
    {
...
#        ifdef DEBUG
            printf("Th %d > i = %d, data = %4.3f, bin = %d\n", my_rank, i, data[i], bin);
#        endif
...
#           ifdef DEBUG
                printf("Th %d > Adding %d to bin_counts[%d] = %d\n", my_rank, loc_bin_counts[j*bin_count + i], i, bin_counts[i]);
#           endif
